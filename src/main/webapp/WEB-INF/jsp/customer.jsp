...
<body>
						...
						<div class="col-md-12 text-right">
							...
							<ul class="pagination">
								...
								<c:if test="${pageInfo.hasPreviousPage}">
									<li>
										<a href="page?pageInfo=${pageInfo.pageNum-1}&custName=${custName}&custSource=${custSource}&custIndustry=${custIndustry}&custLevel=${custLevel}">&laquo;</a>
									</li>
								</c:if>
								<c:if test="${!pageInfo.hasPreviousPage}">
									<li>
										<a href="javascript:;"  class="btn btn-info disabled" role="button">&laquo;</a>
									</li>
								</c:if>

								<c:forEach items="${pageInfo.navigatepageNums }" var="page_index">

									<c:if test="${pageInfo.pageNum==page_index }">
										<li class="active"><a href="javascript:;">${page_index}</a></li>
									</c:if>
									<c:if test="${pageInfo.pageNum!=page_index }">
										<li><a href="page?pageInfo=${page_index}&custName=${custName}&custSource=${custSource}&custIndustry=${custIndustry}&custLevel=${custLevel}">${page_index }</a></li>
									</c:if>
								</c:forEach>
								...
								<c:if test="${pageInfo.hasNextPage }">
									<li>
										<a href="page?pageInfo=${pageInfo.pageNum+1}&custName=${custName}&custSource=${custSource}&custIndustry=${custIndustry}&custLevel=${custLevel}">&raquo;</a>
									</li>
								</c:if>
								<c:if test="${!pageInfo.hasNextPage }">
									<li>
										<a href="javascript:;"  class="btn btn-info disabled" role="button">&raquo;</a>
									</li>
								</c:if>
							</ul>
						</div>
	...
	<script type="text/javascript">
		...
		function updateCustomer() {
			$.post("/page/update",$("#edit_customer_form").serialize(),function(data){
				alert("客户信息更新成功！");
		...
		function deleteCustomer(id) {
			if(confirm('确实要删除该客户吗?')) {
				$.post("/page/delete",{"id":id},function(data){
					alert("客户删除更新成功！");
					window.location.reload();
				});
			}
